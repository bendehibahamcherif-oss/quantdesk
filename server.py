...
def find_html():
    ...
    for name in ["quant-desk.html", "index.html"]:
        if os.path.exists(os.path.join(HTML_DIR, name)):
            return name
    # Fallback: latest quant-desk*.html
    matches = glob.glob(os.path.join(HTML_DIR, "quant-desk*.html"))
    if matches: return os.path.basename(sorted(matches)[-1])
    matches = glob.glob(os.path.join(HTML_DIR, "*.html"))
    if matches: return os.path.basename(sorted(matches)[-1])
    return None

# HTML_FILE resolved dynamically per request

@app.route("/")
@require_auth
def index():
    html_file = find_html()
    if not html_file: return "HTML non trouvé — placez un fichier quant-desk.html dans le dossier du serveur", 404
